...
//int get_keys_processed=0;
//int add_keys_processed=0;

//pthread_mutex_t add_put_lock;;

//pthread_cond_t add_put_cond;

//////
//int readers_count=0;
//int writer_wants_to_write=0;

//pthread_mutex_t mutex = PTHREAD_MUTEX_INITIALIZER;
//pthread_cond_t readers_cv = PTHREAD_COND_INITIALIZER;
//pthread_cond_t writers_cv = PTHREAD_COND_INITIALIZER;
...
int db_add(DB* self, Variant* key, Variant* value) {
    ...
    pthread_mutex_lock(&self->lock_rw);

    // Flag 
    self->writer_wants_to_write = 1;

    // Wait until there are no readers
    while (self->readers_count > 0) {
        pthread_cond_wait(&self->writers_cv, &self->lock_rw);
    }
    ...
    printf("\033[1;35m #Thread with id %lu wrote #%d key\033[0m\n\n\n", pthread_self(), self->add_keys_processed);
    self->add_keys_processed++;


    self->writer_wants_to_write = 0;

    pthread_cond_broadcast(&self->readers_cv);
    pthread_cond_broadcast(&self->writers_cv);

    pthread_mutex_unlock(&self->lock_rw);
...
int db_get(DB* self, Variant* key, Variant* value) {
    ...
    while (self->writer_wants_to_write) {
        pthread_cond_wait(&self->readers_cv, &self->lock_rw);
    }
    ...
    if (memtable_get(self->memtable->list, key, value) == 1) {
        printf("\033[1;36m #Thread with id %lu found #%d key in MEMTABLE\033[0m\n\n\n", pthread_self(), self->get_keys_processed);
        self->get_keys_processed++;
        ret_value = 1;
    } else {
        ret_value = sst_get(self->sst, key, value);
        printf("\033[1;36m #Thread with id %lu found #%d key in SST\033[0m\n\n\n", pthread_self(), self->get_keys_processed);
        self->get_keys_processed++;
    }
